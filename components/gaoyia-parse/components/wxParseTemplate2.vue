<template>
	...
	<block v-if="node.node == 'element'">
		<!--button类型-->
		<view v-if="node.tag == 'button'" type="default" size="mini" :class="node.classStr" :style="node.styleStr">
			<wx-parse-template :node="node" />
		</view>
		
		<!--a类型-->
		<view v-else-if="node.tag == 'a'" @tap="wxParseATap(node.attr,$event)" :class="node.classStr" :data-href="node.attr.href" :style="node.styleStr">
