<template name="zy-search">
	<view>
		<view class="search">
			<!-- #ifdef APP-PLUS -->
				<image src="../../static/zy-search/voice.svg" mode="aspectFit" @tap="startRecognize()" class="voice-icon"></image>
			<!-- #endif -->
			...
			<image src="/static/ic_search@2x.png" mode="aspectFit" @tap="searchStart()" class="search-icon"></image>
		</view>
		<template v-if="isBlock">
			<view class="s-block" v-if="hList.length > 0">
				<view class="header">
					历史记录
					<!-- <image src="../../static/zy-search/delete.svg" mode="aspectFit" @tap="delhistory"></image> -->
				</view>
				<view class="list">
					<view v-for="(item,index) in hList" :key="index" @tap="keywordsClick(item)">{{item}}</view>
				</view>
			</view>
			<view class="wanted-block" v-if="showWant">
				<view class="header">猜你想搜的</view>
				<view class="list">
					<view v-for="(item,index) in wantList" :key="index" @tap="keywordsClick(item)">{{item}}</view>
				</view>
			</view>
		</template>
		<template v-else>
