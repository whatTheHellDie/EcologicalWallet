<template>
	<view class="wrap-content wrap">
		<sub-nav :title="title">
			<view class="text" @click="toRecord">记录</view>
		</sub-nav>
		<view>
			<view class="row b-b">
				<text class="tit">账户类型</text>
				<view class="input" v-if="withdrawType==1">币账户(TC)</view>
				<view class="input" v-if="withdrawType==2">币账户(TB)</view>
			</view>
			<view class="row b-b">
				<text class="tit">账户余额</text>
				<view class="input" v-if="withdrawType==1">{{userInfo.lovebirdAccount.toFixed(2)}}</view>
				<view class="input" v-if="withdrawType==2">{{userInfo.tbCurrencyNum.toFixed(2)}}</view>
			</view>
			
			<view class="row b-b">
				<text class="tit">提币数量</text>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.withdrawType=option.withdrawType
		},
		onShow(){
			this.wallet
		},
		...
		methods: {
			...
			toRecord(){
				var type;
				console.log(this.withdrawType==="1")
				...
				this.$toPage('/pages/user/withdrawCoinRecord?status=0&type='+type)
			},
			getUserName() {
				uni.request({
