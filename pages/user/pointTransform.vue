	...
	<view class="wrap-content wrap">
		...
		<view v-if="coinType==1">
			...
			<view class="row b-b">
				...
				<view class="input">{{userInfo.lovebirdPoint.toFixed(2)}}</view>
			</view>
			<view class="row b-b">
		...
		<view v-if="coinType==2">
			...
			<view class="row b-b">
				<text class="tit">账户类型</text>
				<view class="input">积分(Tree Bank)</view>
			</view>
			<view class="row b-b">
				<text class="tit">账户余额</text>
				<view class="input">{{userInfo.lovebirdPoint.toFixed(2)}}</view>
		...
		methods: {
			...
			confirm() {
				if(this.coinType==1){
					...
					this.transformLoveBird()//积分转成(Tree Bank)币
				}else if(this.coinType==2){
					this.transferLoveBird()//积分转账
				}
			}
		}
	}
