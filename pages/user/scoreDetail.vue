	...
	<view class="wrap">
		...
		<view class="score-wrap">
			<view class="left">
				...
				<view class="title" v-if="accountType==3">币账户(TC):</view>
				<view class="title" v-if="accountType==4">币账户(USDT):</view>
				<view class="title" v-if="accountType==5">币账户(TB):</view>
				<view class="title" v-if="accountType==6">积分(TP):</view>
				<div class="number">{{account?account.toFixed(2):"0.00"}}</div>
			</view>
			<!-- <view class="left active" v-if="accountType==3||accountType==4">
				<view class="title">冻结账户:</view>
				<div class="number">{{freezeAccount?freezeAccount.toFixed(2):"0.00"}}</div>
			</view> -->
		</view>
		<view class="detail-list">
			<view class="ul" v-if="!noData">
				<view class="li" v-for="item in detailList">
					...
					<view class="name">余额<text class="text">{{item.balance?item.balance.toFixed(2):""}}</text></view>
	...
	export default {
		...
		methods: {
			...
			getDetail() {
				...
				//明细
				uni.request({
					url: this.$httpUrl("/app/about/capital/financialFlow"),
					data: {
