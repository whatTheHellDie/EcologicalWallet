<template>
	<view class="wrap">
		...
		<view class="detail-list" :style="{paddingTop:'130rpx'}">
			<view class="ul" v-if="!noData">
				<view class="li" v-for="item in detailList">
					<view class="name">编号<text class="text">{{item.number}}</text></view>
					<view class="name">类型<text class="text active">{{item.type==1?"充值":"提币"}}</text></view>
					<view class="name">币种<text class="text">TC(积分)</text></view>
					<view class="name" v-if="item.type==1">充值数额<text class="text">{{item.payment?item.payment.toFixed(2):"0.00"}}</text></view>
					<view class="name"  v-if="item.type==2">到账数额<text class="text">{{item.account?item.account.toFixed(2):"0.00"}}</text></view>
					<view class="name">时间<text class="text">{{item.creatTime}}</text></view>
				</view>
			</view>
		</view>
	...
	export default {
		data() {
			return {
				...
				showList:[
					...
					[{name:"全部",type:0},{name:"充值",type:1},{name:"提币",type:2}],
		...
		methods: {
			...
			getDetail() {
				...
				//明细
				uni.request({
					url: this.$httpUrl("/app/about/holdthecode/gameRecharge/record"),
					data: {
					...
					success: (res) => {
						...
						if (res.data.code === "0") {
							...
							if(data.length!==0){
							if(this.pageNum===1&&data.list==0){
								this.noData=true
							}
							this.loadingPage=false
							if(data.list.length<20&&!this.noData){
