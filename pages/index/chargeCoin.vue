<template>
	<view class=" wrap">
		<sub-nav :title="title">
			<view class="text" @click="$toPage('/pages/index/chargeCoinRecord?status=0')">记录</view>
		</sub-nav>
		<view>
			<view class="row b-b">
				<text class="tit">账户类型：</text>
				<view class="input">USDT</view>
			</view>
			<view class="row b-b">
			...
			<view class="wallet-address">
				...
				<view class="qr-wrap">
					<image :src="QRCode" class="qr-img" mode=""></image>
				...
				uni.showLoading({
					...
					uni.request({
						...
						success: (res) => {
							...
							if (res.data.code === 0) {
								uni.redirectTo({
								    url: '/pages/index/chargeCoinRecord?status=0'
								});
							} else {
