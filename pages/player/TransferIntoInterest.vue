<template>
	<view class="wrap-content wrap">
		...
		<view class="row b-b into-interest">
			<text class="tit">转入合约数额：</text>
			<input class="interest-input" type="text" v-model="scoreNumber" placeholder="请输入需要合约的TC积分数额" placeholder-class="placeholder" />
...
<script>
	...
	export default {
		...
		methods: {
			...
			toTransfer(){
				uni.request({
					url: this.$httpUrl("/app/about/holdthecode/deposits/in"),
					data: {
						userId:getApp().globalData.userId,
